...
public:
    ...
    LocalSequenceAligner(const std::string &, const std::string &, const int = 1, const int = -1, const int = -2);
...
LocalSequenceAligner::LocalSequenceAligner()
{
}

LocalSequenceAligner::LocalSequenceAligner(const std::string &_A, const std::string &_B,
...
void LocalSequenceAligner::init_matrix()
{
    for (int i = 0; i < M.size(); i++)
    {
        M[i][0].value = 0;
        M[i][0].came_from = DONE;
    }
    for (int i = 0; i < M[0].size(); i++)
    {
        M[0][i].value = 0;
        M[0][i].came_from = DONE;
    }
}

void LocalSequenceAligner::print_matrix(const int padding)
{
    std::cout << "\n*** Displaying Matrix ***\n" << std::endl;
...
State LocalSequenceAligner::get_max_state(const int &match, const int &top_gap, const int &left_gap)
{
...
void LocalSequenceAligner::align()
{
...
void LocalSequenceAligner::copy_cur_state(std::vector<std::stack<char>> &AlignmentAs,
...
void LocalSequenceAligner::push_nucleotids(  std::stack<char> &A,
                            std::stack<char> &B,
...
void LocalSequenceAligner::get_best_alignments(const char gap)
{
    /*
        Solution
            -TCGAA
    ...
    */
    ...
    int max_value = -1e9;
    size_t max_value_idx_i, max_value_idx_j;

    for(size_t i = 0; i < M.size(); i++)
    {
        for(size_t j = 0; j < M[i].size(); j++)
        {
            if(M[i][j].value > max_value)
            {
                max_value = M[i][j].value;
                max_value_idx_i = i;
                max_value_idx_j = j;
            }
        }
    ...
    std::vector<size_t> indexAs = {max_value_idx_i};
    std::vector<size_t> indexBs = {max_value_idx_j};
...
int main()
{
    std::string A = "ATCAGAA";
    std::string B = "ATCAGAAAT";

    LocalSequenceAligner Aligner(A, B);
    Aligner.align();
    Aligner.print_matrix(2);
